...
- `blockTimestamp` stores a cropped version of `block.timestamp` (uint256),
  - It is truncated to uint32 for storage packing efficiency
...
Eventually, a newer timestamp will be numerically smaller than an older one after wraparound.
  - This code was written for Solidity 0.5.16, where unsigned arithmetic naturally wrapped instead of reverting on overflow.
  - Interval between the two observations must be less than $2^{32}$ seconds for calculation to work (almost sure)
  - Because subtractions wrap to zero after max int, it returns the correct number of elapsed seconds


> Uniswap’s oracle logic only cares about differences between observations, that is why this calculation works.

**Example**

$blockTimestamp_1 = 2^{32} - 5$
...
| Total bits | 224                                        |
| Min value  | $0$                                        |
| Max value  | $2^{112} − 2^{−112} \approx +5.19×10^{33}$ |
| Resolution | $2^{−112} \approx 1.926×10^{-34}$          |

This is an enormous format

- it gives you a range on the scale of $5×10^{33}$
...
<div className="codeBlock">
  ...
  $\text{TWAP}_0^{Q112x112} = \frac{\Delta\text{price0Cumulative}}{\text{timeElapsed}}$

  $\text{TWAP}_0^{decimal} = \frac{\text{TWAP}_0^{Q112x112}}{2^{112}}$
