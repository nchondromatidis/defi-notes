...
#### Calculate the average price for block M
<div className="codeBlock">
$N$ = prior block, $M$ = target block

$\Delta\texttt{price0Cumulative} = \texttt{price0CumulativeLast}_M - \texttt{price0CumulativeLast}_N$

$\text{timeElapsed} = (\texttt{blockTimestampLast}_M \mod 2^{32}) - (\texttt{blockTimestampLast}_N \mod 2^{32})$

$\text{TWAP}_0 = \frac{\Delta\texttt{price0Cumulative}}{\text{timeElapsed}} \times 2^{112}$
