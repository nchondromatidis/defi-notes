...
Fixed-point numbers store a scaled integer, if the real value is $x$, the stored value usually is:
...
- **One’s complement:** negative values are formed by inverting all bits of the positive value; it has both $+0$ and $-0$.
- **Sign-magnitude:** the most significant bit is an explicit sign bit, and the remaining bits store the magnitude; it also has both $+0$ and $-0$.

For **Unsigned storage** there is no sign encoding, so negative values are not representable.

### Ranges

If an $N$-bit stored integer

- signed, that use two’s complement,
  - its integer range is $-2^{N-1}$ to $2^{N-1}-1$.
  - its fixed points range is $\left[\frac{-2^{N-1}}{\text{scale}},\ \frac{2^{N-1}-1}{\text{scale}}\right]$
- unsinged
  - its integer range is $0 \text{ to } 2^N - 1$
  - its fixed points range is $0 \text{ to } \frac{2^N - 1}{scale}$
